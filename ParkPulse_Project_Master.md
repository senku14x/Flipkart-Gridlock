...
- *2026-06-22:* **OSM cross-check run for real + integrated.** User ran `enrich_osm.py --no-betweenness` + `osm_validate.py` in Colab against the real Bengaluru OSM extract (2,534 cells), pushed `data/hex_osm.csv` (+ `osm_validation.md`, `fig-osm.png`) to the branch. Refined `osm_validate.py` to lead with the strong, honest signal: a **dose-response** (cells near a market/shop/transit climb from **34%** in the lowest impact band to **62%** in the highest), **markets 17% of top-30 vs 2% citywide (~8x)**, and generator-proximity tracking impact ~2x stronger than road class (Spearman 0.23 vs 0.12). The weak arterial correlation (0.12) is reported as *the finding*, not a miss: parking chokes narrow commercial streets, not flyovers, so a parking-congestion score *should* point away from arterials. Wired into the LaTeX/solution doc (new validation subsection + `fig-osm` + results row), the `.pptx` deck (new "Does it agree with the real city?" slide, now 17 slides), `PITCH_DECK.md`, `SUBMISSION.md` description, README, and the web methodology card; regenerated all docx + the Overleaf zip; re-ran `prepare_data.py` so the map hover now shows OSM road + POI context for all 2,534 cells. Verified: web builds clean, LaTeX lints clean, no em-dashes.
