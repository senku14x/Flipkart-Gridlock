...
>
> ⚠️ **Data does not persist across chats or sandbox resets.** When resuming, re-upload the dataset zip (`jan_to_may_police_violation_anonymized791b166_csv.zip`).

**Status legend:** ✅ Done · 🟡 In progress · ⬜ Not started · 🔵 Needs your action (outside the sandbox)
...
| **One-line pitch** | AI-driven parking intelligence that finds chronic illegal-parking hotspots, scores each by its drag on traffic flow, and tells enforcement exactly *where and when* to deploy for the most congestion relief per patrol-hour. |
...
**⚠️ The defining gap:** there is **no traffic-flow data** — no speeds, volumes, or travel times. So "impact on traffic flow" must be **derived**, not looked up. (See §3 — this is our opening, not a weakness.)
...
**f) Product.** Interactive map centerpiece — impact-weighted hex heatmap with a **raw-count ↔ impact-weighted toggle** (the contrast sells it), time-of-day slider, junction/zone layers — plus dashboard (ranked-zone table, hour×day heatmap, breakdowns, patrol view, KPI cards).
...
|---|-------|:---:|---|---|:---:|
| 1 | **Title** | ✸ | Clear descriptive title | Recommended below | 🟡 |
| 2 | **Description** | ✸ | Project write-up (formatting/links ok) | Draft v0 below; refine after build | 🟡 |
| 3 | **Theme** | ✸ | Pick one | "Poor Visibility on Parking-Induced Congestion" | ✅ |
...
- [✅] **5. Patrol optimizer + Pareto analysis** — `scripts/patrol_optimizer.py` → `outputs/patrol_plan.md/.csv`, `pareto.png`. Greedy max-coverage (beat = cell + H3 ring): 20 beats cover 53% of exposure-weighted impact vs 47% naive; Pareto (top 1% of cells = 35%, 58 cells = 50%); forecaster-driven next-day plan (14/20 beats shared with static).
...
- [✅] **7. Prediction layer** (temporal holdout) — `scripts/forecast.py`. Multi-model GBM bake-off (LightGBM/XGBoost/CatBoost/HistGBM, Tweedie/Poisson) on 906 hotspot cells; beats seasonal-naive (+36% coverage@20) + dominates on calibration. Best model saved to `outputs/forecast_model.pkl`.
- [✅] **8. Packaged dashboard / demo artifact** — `web/`: custom **Next.js (static export) + deck.gl + Recharts** site (not Streamlit — can't run on Vercel). Impact map (raw↔impact toggle, hover breakdown), ranked zones, forecaster (bake-off + ablation), live patrol optimizer (N-beats slider), methodology. Fully static (precomputed JSON), deploys to Vercel with no backend/tokens.
...
- *2026-06-21* — **Step 8 built (interactive web app).** `web/` — a custom **Next.js 16 (static export) + deck.gl 9 (`H3HexagonLayer`) + Recharts** site, Tailwind v4, dark theme. Decision: **not Streamlit** — Streamlit needs a persistent server and can't run on Vercel; a static custom frontend is both better-looking and trivially CDN-scalable (all compute is precomputed). Sections: hero/KPIs, GPU impact map (raw↔impact toggle + hover breakdown), ranked zones, forecaster (bake-off bars + ablation), live patrol optimizer (N-beats slider driving the greedy-vs-naive coverage chart + deployment plan), methodology/honesty. Data via `web/prepare_data.py` → `web/public/data/*.json` (reuses `patrol_optimizer.py` so the site matches the repo). Verified: builds static + renders headless with zero runtime errors (deck.gl on React 19). Deploy: Vercel, Root Directory = `web`, no env/tokens. Next: deploy + Step 9 deliverables (deck, video, screenshots, source zip).
- *2026-06-21* — **Step 5 built (patrol optimizer + Pareto).** `scripts/patrol_optimizer.py` → `outputs/patrol_plan.md/.csv`, `pareto.png`. Pareto on the exposure-weighted additive impact mass (`impact_sum`): top 1% of cells = 35% of impact, 58 cells (2.3%) = 50%, 269 = 80%. Greedy max-coverage optimizer where each patrol works a beat (cell + H3 ring-1): because the worst cells clump in commercial cores, greedy spreads beats and **20 beats cover 53% vs 47% for naive top-N** (+6 pts, same fleet). Each beat carries its exposure-weighted enforcement window (where + when). Bonus: a forecaster-driven next-day plan (predicted violations × impact/violation) that shares 14/20 beats with the static plan and reallocates the rest to predicted surges — the reactive→predictive thesis made operational. Next: Step 8 (Streamlit dashboard).
