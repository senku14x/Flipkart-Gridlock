...
it two ways — never claiming "accuracy":
  ...
  2. STABILITY — re-score each calendar month INDEPENDENTLY (same pipeline) and
     ...
     A score that only reflects noise would not reproduce month to month.
...
def write_markdown(fv: pd.DataFrame, stab: dict, path: str = OUT_MD) -> None:
    ...
    L += [
        "",
        "These are exactly the dense market/commercial cores the brief calls out — "
        "strong qualitative evidence the score tracks real congestion pressure, not artefacts.",
        "",
    ...
    L += [
        "",
        "High month-to-month rank correlation + large top-50 overlap means the hotspot "
        "ranking is **structural, not noise** — the same streets stay bad. That stability "
        "is the robustness evidence we *can* offer in the absence of a flow-feed label.",
        "",
        "---",
        "*Generated by `scripts/face_validity.py`. Impact remains a transparent engineered "
        "index designed to become supervised once a speed feed provides labels.*",
        "",
    ]
