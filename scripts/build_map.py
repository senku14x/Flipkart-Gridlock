...
+ LayerControl). The contrast shows how impact weighting re-ranks the city vs.
raw violation density (e.g. a cell with high counts logged during the pre-dawn
enforcement sweep stays "hot" on the raw layer but cools on the impact layer,
per CLAUDE.md §6.6).
...
def _add_titlebar(m: folium.Map, df: pd.DataFrame) -> None:
    ...
    <div style="position:fixed; top:12px; left:60px; z-index:9999; width:430px;
      ...
      <div style="font-size:12px; color:#333; margin-top:5px;">
        ...
        <b>impact-weighted</b> and <b>raw violation density</b>: the re-ranking is the main comparison.
