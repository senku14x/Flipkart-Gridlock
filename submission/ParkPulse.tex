...
\subsection{An independent cross-check against real geography}
The score uses no road network and no land use, only the violation feed, so we can cross-check it against real OpenStreetMap geography it never saw (\texttt{osm\_validate.py}). It rediscovers the city's commercial cores: sorting all 2{,}534 cells into ten equal impact bands, the share next to a market, shopfront, or transit stop climbs steadily from 34\% in the lowest band to 62\% in the highest. The top-30 hotspots sit next to a marketplace 17\% of the time versus 2\% city-wide (an $\sim$8x enrichment), and proximity to these congestion-generators tracks impact about twice as strongly as road class (Spearman 0.23 vs 0.12). The weak alignment with arterial class is itself the finding: parking does not choke the wide arterials and flyovers, it chokes the narrow commercial streets feeding them, so a score built to find parking-induced congestion \emph{should} point away from the arterials. Derived with none of this data, the score lines up with the commercial geography that drives curbside demand, the closest thing to ground truth without a speed feed.

\begin{figure}[htbp]\centering
\includegraphics[width=0.92\linewidth]{fig-osm}
\caption{Independent OSM cross-check. Left: the share of cells near a market, shop, or transit stop climbs with impact (34\% to 62\%). Right: the top hotspots track commerce, not arterial road class.}
\end{figure}
...
OSM cross-check & commercial proximity rises 34\%$\to$62\% with impact \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[label=\textbullet]
  \item \textbf{No ground truth for impact.} The score is an engineered index, not a measurement; we validate by face validity, month-to-month stability, a robustness check on the weights, and an independent OpenStreetMap cross-check that rediscovers the commercial cores it was never shown. We never claim measured accuracy for it.
