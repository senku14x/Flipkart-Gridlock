...
- **Analytics upgrades DONE**: congestion cost (`congestion_cost.py`, ~Rs 5.2 cr/yr est. delay), false-detection classifier (`detection_validity.py`, ROC-AUC 0.758), emerging hotspots (`emerging_hotspots.py`, share-detrended trend over the 4 full months), enforcement gap (`enforcement_gap.py`: 4-5am sweep = 15% of effort but 4% of impact) + patrol ROI (rupees/day relieved on the optimizer slider), optional OSM cross-check (`enrich_osm.py` fetches real road class + k-sampled betweenness + POIs; `osm_validate.py` correlates the score, built without the road network, against them as independent validation; run locally, geo APIs blocked in CI). All surfaced in `web/` (cost / trends / triage sections; cost + trend in the map hover). Dropped an hour-level schedule: pm_peak_share is ~0 citywide (enforcement rarely works evenings), so observed hours reflect patrol shifts, not congestion. Score robustness (`score_robustness.py`): across 2,000 random axis weightings the top-20 ranking holds (median Spearman 0.97, 18/20 retained), so the score isn't a hand-tuned artefact.
